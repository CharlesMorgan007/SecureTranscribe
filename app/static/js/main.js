...
class SecureTranscribe {
    ...
    setupEventListeners() {
        ...
        // Drag and drop
        const uploadArea = document.querySelector('.upload-area') || document.getElementById('audioFile').parentElement;
        if (uploadArea) {
    ...
    validateFile(file) {
        ...
        if (!validTypes.includes(file.type)) {
            this.showError('Invalid file type. Please upload MP3, WAV, M4A, FLAC, or OGG files.');
            return false;
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (result.processing_started) {
                    this.showInfo('Transcription has been started and added to the queue.');
                } else {
    ...
    updateQueueDisplay(status) {
        ...
        document.getElementById('processingJobs').textContent = status.processing_jobs || 0;
        document.getElementById('completedJobs').textContent = status.completed_jobs || 0;
    ...
    displayTranscriptions(transcriptions) {
        ...
        const html = transcriptions.map(trans => this.createTranscriptionCard(trans)).join('');
    ...
    async startTranscription(transcriptionId) {
        try {
            const response = await fetch(`${this.apiBase}/transcription/start/${transcriptionId}`, {
                method: 'POST',
                credentials: 'include'
            });
    ...
    async viewTranscription(transcriptionId) {
        try {
            const response = await fetch(`${this.apiBase}/transcription/status/${transcriptionId}`, {
                credentials: 'include'
            });
    ...
    async exportTranscription(transcriptionId, format) {
        try {
            const response = await fetch(`${this.apiBase}/transcription/export/${transcriptionId}?export_format=${format}`, {
                credentials: 'include'
            });
...
function showSessionInfo() {
    fetch('/api/sessions/current', { credentials: 'include' })
        ...
        .then(session => {
            alert(`Session Information:\n\nSession ID: ${session.session_id}\nCreated: ${new Date(session.created_at).toLocaleString()}\nFiles Processed: ${session.total_files_processed}\nAverage Confidence: ${(session.average_confidence * 100).toFixed(1)}%`);
        })
        .catch(error => {
