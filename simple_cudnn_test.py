#!/usr/bin/env python3
"""
Simple cuDNN Test and Fix Script
Tests and fixes cuDNN library issues in PyTorch
"""
...
def check_cudnn_libraries(cudnn_path):
    ...
    print(f"Exists: {cudnn_path.exists()}")
...
def test_direct_library_loading(cudnn_path):
    ...
    new_ld_path = (
        f"{cudnn_path}:{current_ld_path}" if current_ld_path else str(cudnn_path)
    )
    ...
    # Test Python import with environment
    test_code = f'''
import os
os.environ["LD_LIBRARY_PATH"] = "{new_ld_path}"
...
try:
    ...
    # Try to get cuDNN symbol
    if hasattr(lib, 'cudnnCreateTensorDescriptor'):
        print("✅ cuDNN symbols available")
        return True
    else:
        print("❌ cuDNN symbols not found")
        return False
...
def main():
    """Main function."""
    print("🚀 Simple cuDNN Test and Fix")
    print("=" * 70)
